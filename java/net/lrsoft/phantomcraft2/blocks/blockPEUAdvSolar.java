...
public class blockPEUAdvSolar extends BlockContainer{
	  @SideOnly(Side.CLIENT)
	  private IIcon iconFront;
	  @SideOnly(Side.CLIENT)
	  private IIcon iconTop;
	protected blockPEUAdvSolar(Material material) {
		super(material);
	}
	@Override
	public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_) {
		// TODO Auto-generated method stub
		return new rtTEASPEU();
	}
	private final Random raaaadom = new Random();
	  public void breakBlock(World world, int x, int y, int z, Block block, int meta)
	  {
	    TileEntity te = world.getTileEntity(x, y, z);
	    if ((te != null) && ((te instanceof rtTEASPEU)))
	    {
	    	rtTEASPEU storage = (rtTEASPEU)te;
...
{
  player.openGui(phantomcraft2.instance, 2018222002, world, i, j, k);
 return true;
}
@SideOnly(Side.CLIENT)
public void registerBlockIcons(IIconRegister iconRegister)
{
 this.blockIcon = iconRegister.registerIcon("phtc2:ASPEU_p1");
 this.iconFront = iconRegister.registerIcon("phtc2:ASPEU_p1");
 this.iconTop = iconRegister.registerIcon("phtc2:ASPEU_p2");
}
