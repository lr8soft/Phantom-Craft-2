...
public class blockAPEUStorage extends BlockContainer{
	  @SideOnly(Side.CLIENT)
	  private IIcon iconFront;
	  @SideOnly(Side.CLIENT)
	  private IIcon iconTop;
	protected blockAPEUStorage(Material material) {
		super(material);
	}
	@Override
	public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_) {
		// TODO Auto-generated method stub
		return new rtTEAPEUStorage();
	}
	private final Random raaaadom = new Random();
	  public void breakBlock(World world, int x, int y, int z, Block block, int meta)
	  {
	    TileEntity te = world.getTileEntity(x, y, z);
	    if ((te != null) && ((te instanceof rtTEAPEUStorage)))
	    {
	    	rtTEAPEUStorage storage = (rtTEAPEUStorage)te;
...
{
    player.openGui(phantomcraft2.instance, 2018223001, world, i, j, k);
   return true;
}
@SideOnly(Side.CLIENT)
public void registerBlockIcons(IIconRegister iconRegister)
{
   this.blockIcon = iconRegister.registerIcon("phtc2:APEUS");
   this.iconFront = iconRegister.registerIcon("phtc2:APEUS");
   this.iconTop = iconRegister.registerIcon("phtc2:APEUS");
}
